...
void mmCheckingPanel::sortTable()
{
    ...
    switch (m_listCtrlAccount->g_selected_col)
    {
...
void mmCheckingPanel::updateTable()
{
    ...
    if (m_listCtrlAccount->g_selected_col == TransactionListCtrl::COL_STATUS)
    {
        sortTable();
...
void mmCheckingPanel::CreateControls()
{
    ...
    m_listCtrlAccount->setSortColumn(m_listCtrlAccount->g_selected_col);
    ...
    if (strVal.ToLong(&val)) m_listCtrlAccount->g_selected_col = m_listCtrlAccount->toEColumn(val);
    // --
    ...
    m_listCtrlAccount->setSortColumn(m_listCtrlAccount->g_selected_col);
    m_listCtrlAccount->setSortOrder(m_listCtrlAccount->g_asc);
    m_listCtrlAccount->setColumnImage(m_listCtrlAccount->g_selected_col
...
void TransactionListCtrl::OnColClick(wxListEvent& event)
{
    ...
    setColumnImage(g_selected_col, -1);

    if (g_selected_col == ColumnNr) g_asc = !g_asc; // toggle sort order

    m_sortCol = toEColumn(ColumnNr);
    g_selected_col = toEColumn(ColumnNr);

    setColumnImage(m_sortCol, g_asc ? ICON_ASC : ICON_DESC);
...
void TransactionListCtrl::refreshVisualList(int trans_id, bool filter)
{
    ...
    setColumnImage(g_selected_col, g_asc ? ICON_ASC : ICON_DESC);
